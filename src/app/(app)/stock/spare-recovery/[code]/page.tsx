...
export default async function SpareRecoveryDetail({ params }: Props) {
  ...
  return (
    <div className="w-full max-w-4xl space-y-5">
      <PageTitle sub="ອາໄຫຼ່ທີ່ເບີກອອກໄປແລ້ວ ແຕ່ວຽກຖືກຍົກເລີກ — ເກັບຄືນສາງ">
        ອາໄຫຼ່ຄ້າງນອກສາງ · ໃບຮັບເຄື່ອງ {head.code}
      </PageTitle>

      <Card title="ຂໍ້ມູນໃບຮັບເຄື່ອງ">
        <dl className="grid gap-2.5 text-xs sm:grid-cols-2">
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ລູກຄ້າ:</dt>
            <dd className="text-slate-800">{head.customer || "-"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ເຄື່ອງ:</dt>
            <dd className="text-slate-800">
              {head.product || "-"} {head.model} · <span className="font-bold text-[#790404]">{head.sn || "-"}</span>
            </dd>
          </div>
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ຊ່າງທີ່ຖືອາໄຫຼ່:</dt>
            <dd className="font-medium text-slate-800">{head.technician || "-"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ຜູ້ຂໍຍົກເລີກ:</dt>
            <dd className="text-slate-800">{head.request_cancel || "-"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ອະນຸມັດຍົກເລີກ:</dt>
            <dd className="text-slate-800">{head.cancel_finish || "ຍັງບໍ່ອະນຸມັດ"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="shrink-0 text-slate-500">ສົ່ງເຄື່ອງຄືນລູກຄ້າ:</dt>
            <dd className="text-slate-800">{head.return_complete || "ຍັງບໍ່ສົ່ງຄືນ"}</dd>
          </div>
        </dl>
      ...
      {docs.length > 0 ? (
        ...
        <div className="flex items-center gap-2.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
          ...
          <p className="text-xs font-semibold text-emerald-800">ອາໄຫຼ່ຂອງໃບນີ້ເກັບຄືນສາງຄົບແລ້ວ</p>
        </div>
      )}
    </div>
  );
