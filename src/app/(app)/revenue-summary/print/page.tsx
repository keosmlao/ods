...
/**
 * **ລາຍງານລາຍຮັບປະຈຳປີ — ສະບັບນຳສະເໜີຜູ້ບໍລິຫານ** (ພິມ A4 ໄດ້).
 *
...
export default async function ExecutiveRevenueReport({ searchParams }: Props) {
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 bg-white p-8 text-slate-800 print:p-0">
      <div className="flex items-start justify-between gap-4 border-b-2 border-slate-800 pb-4">
        ...
        <div className="flex shrink-0 items-center gap-2 print:hidden">
          <Link href={`/revenue-summary/print?year=${year - 1}`} className="rounded-lg border px-2 py-1 text-xs">
            {year - 1}
          </Link>
          <Link href="/revenue-summary" className="rounded-lg border px-3 py-1 text-xs font-semibold">
            ກັບໜ້າສະຫຼຸບ
          </Link>
        </div>
      </div>
