              ...
              {rows.map((row) => (
                <RowLink key={row.code} href={`/service/${row.code}`} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <span className="inline-flex items-center gap-1 rounded bg-brand-50 px-1.5 py-0.5 text-[11px] font-semibold text-brand-800">
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${inWarranty(row.warranty) ? "bg-brand-50 text-brand-800" : "bg-slate-100 text-slate-500"}`}>
                      {row.warranty || "-"}
                    </span>
                  </td>
            ...
            {rows.map((row) => (
              <div key={row.code} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
                <div className="flex items-start justify-between gap-2">
                  <Link href={`/service/${row.code}`} className="font-bold text-brand hover:underline">{row.code}</Link>
                  <span className="inline-flex shrink-0 items-center gap-1 rounded bg-brand-50 px-1.5 py-0.5 text-[10px] font-semibold text-brand-800">
                    <CheckCircle2 className="size-3" />
                    {row.completed_at || "-"}
                  </span>
                </div>
