...
export async function GET(request: NextRequest) {
  ...
  const service=workflow==="repair"&&["CI","ST","IH","PS"].includes(requestedService)?requestedService:"";
  ...
  const sql=workflow==="repair"?`select a.code,c.name_1 customer,c.tel phone,a.name_1 product,a.sn,a.p_brand brand,a.p_model model,a.service_type,a.issue,a.emp_code technician,a.time_register registered from tb_product a left join ar_customer c on c.code=a.cust_code where ${config.condition}${service?" and a.service_type=$1":""} order by a.time_register desc`:`select a.code,a.time_register registered,c.name_1 customer,a.appoint_date appointment,a.doc_ref_1 sale_bill,a.item_name product,a.pro_brand brand,a.pro_model model,a.pro_type product_type,a.pro_size product_size,a.tech_code technician,a.user_created creator from ods_tb_install a left join ar_customer c on c.code=a.cust_code where a.cancel_date is null and a.job_finish is null and ${config.condition} order by a.time_register desc`;
  const body=csv((await query(sql,service?[service]:[])).rows); return new NextResponse(body,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="${workflow}-${status}${service?`-${service}`:""}.csv"`}});
}
