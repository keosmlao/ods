          ...
          return (
            <div key={label} className="flex min-w-0 flex-1 flex-col items-center">
              <div className="flex w-full items-center">
                <span className={`h-0.5 flex-1 ${i === 0 ? "opacity-0" : lineDone ? "bg-teal-400" : "bg-slate-200"}`} />
                <span
                ...
                <span className={`h-0.5 flex-1 ${i === labels.length - 1 ? "opacity-0" : i < current && !cancelled ? "bg-teal-400" : "bg-slate-200"}`} />
              </div>
              <span
                className={`mt-1.5 text-center text-[11px] leading-tight ${
                  active ? "font-bold text-teal-700" : done ? "font-medium text-slate-600" : "text-slate-400"
                }`}
              >
                {label}
              </span>
