...
export function ServiceScan({
  ...
  return (
    <div className="w-full space-y-4">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              className="h-14 flex-1 rounded-xl border-2 border-slate-300 px-4 font-mono text-lg tracking-wider outline-none focus:border-brand-600 focus:ring-4 focus:ring-brand-100"
            />
            <button
              type="button"
              onClick={scan}
              disabled={busy || code.trim().length < 3}
              className="inline-flex h-14 shrink-0 items-center justify-center gap-2 rounded-xl bg-brand-700 px-8 font-semibold text-white transition hover:bg-brand-800 disabled:opacity-50"
            >
          ...
          {notFound && (
            <div className="mt-4 rounded-lg border border-brand-orange-400 bg-brand-orange-100 p-3">
              <p className="flex items-start gap-2 text-xs text-brand-900">
              ...
              <button
                ...
                className="mt-2.5 inline-flex h-9 items-center gap-2 rounded-lg bg-brand-orange-700 px-3 text-xs font-semibold text-white hover:bg-brand-orange-700"
              >
          ...
          {scanError && (
            <div className="mt-4 rounded-lg border border-brand-orange-400 bg-brand-orange-50 p-3 text-xs text-brand-orange-700">
              {t.scanErrorMessage}
            </div>
          )}
        </section>
            ...
            <ol className="mt-3 space-y-2.5">
              {[t.step1, t.step2, t.step3].map((step, index) => (
                <li key={step} className="flex gap-2.5 text-xs text-slate-600">
                  <span className="grid size-5 shrink-0 place-items-center rounded-full bg-brand-50 text-[10px] font-bold text-brand-800">
                    {index + 1}
                  </span>
      ...
      {result && (
        <section className="overflow-hidden rounded-xl border-2 border-brand-600 bg-white shadow-sm">
          <header className="flex flex-wrap items-center gap-2 bg-brand-700 px-5 py-2.5 text-white">
          ...
          <div className="space-y-4 p-5">
              ...
              {months !== null ? (
                <p
                  className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold ${
                    inWarranty ? "bg-brand-50 text-brand-800" : "bg-brand-orange-100 text-brand-900"
                  }`}
                >
            ...
            <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
              <button
                ...
                className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-brand-700 px-5 font-semibold text-white transition hover:bg-brand-700"
              >
