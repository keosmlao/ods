        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className="font-bold text-slate-700">{t.serviceListTitle}</h3>
              <Button type="button" tone="neutral" onClick={() => setOpen(false)} className="h-8 px-3 text-xs">
                {t.close}
              </Button>
            </div>
            {services.length === 0 ? (
              <Empty>{t.noServices}</Empty>
            ) : (
              <Table head={[t.colCode, t.colName, t.colUnit, ""]} minWidth={500}>
  ...
  return (
    <div className="w-full space-y-3">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <span>{t.invoiceTitle}</span><span>/</span><span>{t.newDoc}</span>
      </div>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <form action={save}>
          ...
          <div className="flex min-h-12 flex-wrap items-center gap-3 border-b border-rose-100 bg-[#fffafa] px-3 py-2">
            ...
            <button type="submit" formAction={deleteAllInvoiceItems} formNoValidate className="text-xs font-medium text-slate-600 hover:text-slate-900">{t.cancel}</button>
          </div>

          <div className="px-5 pt-7 sm:px-8 lg:px-10">
            <p className="text-xs text-slate-500">{t.invoiceTitle}</p>
            <h1 className="mt-1 text-3xl font-medium tracking-tight text-slate-900">{docNo}</h1>
            <p className="mt-1 text-xs text-slate-400">{t.docNoHint}</p>

            <div className="mt-8 grid gap-x-16 gap-y-6 lg:grid-cols-2">
              <div className="space-y-3">
                <InvoiceField label={t.custLabel} required value={head.cust_name} />
                <InvoiceField label={t.telLabel} value={head.tel} />
                <InvoiceField label={t.addressLabel} value={head.address} />
              </div>
              <div className="space-y-3">
            ...
            <div className="mt-10 flex gap-5 border-b border-slate-200">
              <TabButton active={activeTab === "lines"} onClick={() => setActiveTab("lines")}>{t.tabLines}</TabButton>
              <TabButton active={activeTab === "payment"} onClick={() => setActiveTab("payment")}>{t.tabPayment}</TabButton>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <div className="space-y-3 rounded-lg border border-slate-200 p-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div><label className={labelClass}>{t.currencyLabel}</label><SelectField name="cash_type" value={cashType} onChange={(value) => setCashType(value || "01")} options={[{ value: "01", label: t.baht }, { value: "02", label: t.kip }]} /></div>
                    <div><label className={labelClass} htmlFor="cash_ex">{t.rateLabel}</label><input id="cash_ex" value={exCash} readOnly className={`${inputClass} text-center`} /></div>
                    <div><label className={labelClass} htmlFor="cash_input">{t.amountLabel}</label><input id="cash_input" value={cashValue} onChange={(event) => setCashValue(event.target.value)} autoComplete="off" className={`${inputClass} text-right`} /></div>
                  </div>
                  <p className="text-right text-sm text-slate-600">= {money(amountCash)} {t.baht}</p>
                </div>

                <div className="space-y-3 rounded-lg border border-slate-200 p-4">
                  <p className="font-semibold text-slate-700">{t.bankTransfer}</p>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-3"><label className={labelClass}>{t.accountLabel}</label><div className="flex gap-2"><input value={bank?.name_1 ?? ""} readOnly className={inputClass} aria-label={t.accountNameLabel} /><Button type="button" tone="info" onClick={() => setPickingBank(true)} className="shrink-0"><Search className="size-4" />{t.selectAccount}</Button></div></div>
                    <div><label className={labelClass} htmlFor="bank_ex">{t.rateLabel}</label><input id="bank_ex" value={exBank} readOnly className={`${inputClass} text-center`} /></div>
                    <div><label className={labelClass} htmlFor="bank_input">{t.amountLabel}</label><input id="bank_input" value={bankValue} onChange={(event) => setBankValue(event.target.value)} readOnly={!bank} autoComplete="off" className={`${inputClass} text-right`} /></div>
                    <div><label className={labelClass} htmlFor="payment_image">{t.transferImage}</label><input id="payment_image" type="file" name="payment_image" accept="image/*" className="w-full text-xs file:mr-2 file:rounded file:border-0 file:bg-slate-100 file:px-2 file:py-2" /></div>
                  </div>
            ...
            <div className={activeTab === "details" ? "grid gap-8 py-6 md:grid-cols-[1fr_auto]" : "hidden"}>
              <dl className="grid gap-x-8 gap-y-3 text-sm sm:grid-cols-2">
                <Field label={t.warrantyLabel} value={head.warranty} />
                <Field label={t.initialIssueLabel} value={head.issue} />
                <Field label={t.techIssueLabel} value={head.issue_2} />
                <Field label={t.technicianLabel} value={head.emp_code} />
                <Field label={t.accessoriesLabel} value={head.p_access} />
                <Field label={t.receiverLabel} value={head.user_regis} />
              </dl>
        ...
        <div className={activeTab === "lines" ? "px-5 pb-8 sm:px-8 lg:px-10" : "hidden"}>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] border-collapse text-xs">
              <thead><tr className="border-b border-slate-200 text-left text-slate-500"><th className="w-9 px-2 py-3" /><th className="px-3 py-3 font-medium">{t.colProductService}</th><th className="px-3 py-3 text-center font-medium">{t.colUnitShort}</th><th className="px-3 py-3 text-center font-medium">{t.qty}</th><th className="px-3 py-3 text-right font-medium">{t.price}</th><th className="px-3 py-3 text-center font-medium">VAT</th><th className="px-3 py-3 text-right font-medium">{t.colValue}</th><th className="px-3 py-3" /></tr></thead>
              <tbody>
                {cart.map((row, index) => <LineRow key={row.roworder} row={row} index={index} productCode={head.code} />)}
                {cart.length === 0 && <tr><td colSpan={8} className="py-12 text-center text-sm text-slate-400">{t.noLines}</td></tr>}
              </tbody>
            </table>
          </div>
      ...
      {pickingBank && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setPickingBank(false)}>
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className="font-bold text-slate-700">{t.accountListTitle}</h3>
              <Button type="button" tone="neutral" onClick={() => setPickingBank(false)} className="h-8 px-3 text-xs">
                {t.close}
              </Button>
            </div>
            <Table head={[t.colCode, t.colName, t.colCurrency, ""]} minWidth={600}>
              {banks.map((row) => (
