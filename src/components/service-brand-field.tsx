...
const field =
  "h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-100";
    ...
    <div>
      <div className="flex gap-2">
        ...
        <button
          ...
          className="grid size-10 shrink-0 place-items-center rounded-lg bg-brand-500 text-white transition hover:bg-brand-500"
        >
        ...
        <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-2">
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={pending}
              onClick={() => void save()}
              className="inline-flex h-10 shrink-0 items-center gap-2 rounded-lg bg-brand-700 px-3 text-sm font-semibold text-white disabled:opacity-60"
            >
              {pending ? <LoaderCircle className="size-4 animate-spin" /> : null}
              ບັນທຶກ
            </button>
          </div>
          {error && <p className="mt-1 text-xs text-brand-orange-700">{error}</p>}
        </div>
      )}
    </div>
  );
