...
export function ReportScheduleManager({ schedule }: { schedule: ReportSchedule[] }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <p className="mb-2 flex items-center gap-1.5 text-xs font-bold text-slate-600"><Clock className="size-4 text-brand-700" /> ເລືອກລາຍງານ + ເວລາສົ່ງ</p>
        ...
        {REPORT_META.map((r) => {
          ...
          return (
            <li key={r.key} className="flex flex-wrap items-center gap-2 py-2 text-sm">
              <input type="checkbox" checked={enabled} disabled={pending} onChange={(e) => save(r.key, e.target.checked, time)} className="size-4 accent-brand-700" />
              ...
              <input type="time" value={time} disabled={pending} onChange={(e) => save(r.key, enabled, e.target.value)} className="h-8 rounded-lg border border-slate-300 px-2 text-sm outline-none focus:border-brand-600" />
            </li>
          );
        })}
      </ul>
