  ...
  const btn = (key: TabKey, label: string, Icon: typeof Wrench) => (
    <button
      ...
      className={`group inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3.5 py-2 text-sm font-bold transition-all duration-200 sm:px-4 ${
        tab === key
          ? "bg-gradient-to-r from-brand-900 to-brand-900 text-white shadow-md shadow-brand-900/15 ring-1 ring-white/10"
          : "text-slate-500 hover:-translate-y-0.5 hover:bg-white hover:text-slate-800 hover:shadow-sm"
      }`}
    >
      <span className={`grid size-7 place-items-center rounded-lg transition-colors ${tab === key ? "bg-white/10" : "bg-white ring-1 ring-slate-200 group-hover:bg-brand-50 group-hover:text-brand-800 group-hover:ring-brand-100"}`}>
