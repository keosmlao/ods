...
export function LoginForm({ t }: { t: Dictionary["login"] }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <button disabled={pending} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand-700 font-semibold text-white shadow-lg shadow-brand-700/20 hover:bg-brand-800 disabled:opacity-60">{pending?<LoaderCircle className="size-5 animate-spin"/>:<ArrowRight className="size-5"/>}{pending?t.submitPending:t.submit}</button>
  </form>;
}
