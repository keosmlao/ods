...
import { claimScopeText, claimStatusText, claimTypeText } from "@/lib/claim-shared";
...
export async function ClaimsView({
  ...
  const t = dict.claimList;
  /** ປ້າຍ ປະເພດ/ສະຖານະ/ຂອບເຂດ (ເບິ່ງ claim-shared — ຄ່າຄົງທີ່ຍັງໃຊ້ຝັ່ງ email/ລາຍງານ) */
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-700">
            <span className="font-mono text-brand-800">CLM-{type}</span> · {claimTypeText(CL, type)}
          </h1>
          <p className="mt-0.5 text-xs text-slate-500">
            {fill(t.openClosedTotal, { open: openN, closed: closedN, total })}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Link href={type === "B" ? "/service/new?kind=claim" : `/claims/new?type=${type}`} className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-brand-700 px-3 text-xs font-semibold text-white hover:bg-brand-800">
            <FilePlus2 className="size-4" /> {type === "B" ? t.receiveClaimGoods : t.openClaim}
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
          <p className="text-[11px] font-semibold text-slate-500">{t.cardOpen}</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-amber-700">{summary.openCount}</p>
          <p className="text-xs text-slate-400">
            {t.outstanding} <b className="tabular-nums text-amber-700">{money(summary.openAmount)}</b> {t.baht}
          </p>
        </div>
        ...
        {type === "A" && (
          <>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardWaitSupplier}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">
                {(counts.sent ?? 0) + (counts.review ?? 0)}
              </p>
              <p className="text-xs text-slate-400">
                {t.sentCount} <b className="tabular-nums">{counts.sent ?? 0}</b> · {t.reviewingCount}{" "}
                <b className="tabular-nums">{counts.review ?? 0}</b>
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardApproved}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">{counts.approved ?? 0}</p>
              <p className="text-xs text-slate-400">
                {t.amount} <b className="tabular-nums">{money(summary.approvedAmount)}</b> {t.baht}
              </p>
            </div>
          </>
        )}
        ...
        {type === "B" && (
          <>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardScope}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">{scopeSummary.scope.whole ?? 0}</p>
              <p className="text-xs text-slate-400">
                {t.wholeAndPart} <b className="tabular-nums">{scopeSummary.scope.part ?? 0}</b> {t.docsUnit}
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardFulfillment}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">{scopeSummary.fulfillment.supplier ?? 0}</p>
              <p className="text-xs text-slate-400">
                stock <b className="tabular-nums">{scopeSummary.fulfillment.stock ?? 0}</b> · {t.buyNew}{" "}
        ...
        {type === "C" && (
          <>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardNotNotified}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">{counts.notify ?? 0}</p>
              <p className="text-xs text-slate-400">
                {t.jobsReady} <b className="tabular-nums">{candidates.length}</b> {t.jobsUnit}
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[11px] font-semibold text-slate-500">{t.cardNotified}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-slate-700">{counts.notified ?? 0}</p>
              <p className="text-xs text-slate-400">
                {t.amount} <b className="tabular-nums">{money(summary.notifiedAmount)}</b> {t.baht}
              </p>
            </div>
          </>
        )}
        ...
        {type !== "B" && (
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            {type === "A" ? (
              <>
                <p className="text-[11px] font-semibold text-slate-500">{t.cardOldest}</p>
                <p className={`mt-1 text-2xl font-bold tabular-nums ${summary.oldestDays >= 30 ? "text-red-600" : summary.oldestDays >= 7 ? "text-amber-700" : "text-slate-700"}`}>
                  {summary.oldestDays} {t.days}
                </p>
                <p className="text-xs text-slate-400">
                  {t.claimingSuppliers} <b className="tabular-nums">{summary.suppliers}</b> supplier
                </p>
              </>
            ) : (
              <>
                <p className="text-[11px] font-semibold text-slate-500">{t.cardPaid}</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-emerald-700">{counts.paid ?? 0}</p>
                <p className="text-xs text-slate-400">
                  {t.amount} <b className="tabular-nums">{money(summary.paidAmount)}</b> {t.baht}
                </p>
              </>
            )}
          </div>
        ...
        {type === "B" && (
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <p className="text-xs text-slate-400">
              {t.checkingCount} <b className="tabular-nums">{counts.checking ?? 0}</b> {t.docsUnit}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <Link href={link()} className={`rounded-lg px-2.5 py-1 text-[11px] font-semibold ${!status ? "bg-brand-700 text-white" : "bg-white text-slate-500 ring-1 ring-slate-200 hover:bg-slate-50"}`}>
          {t.all} <span className="tabular-nums opacity-80">{total}</span>
        </Link>
      ...
      {type === "C" && candidates.length > 0 && (
        <div className="rounded-2xl border border-brand-orange-400 bg-brand-orange-100/60 p-4">
          <p className="mb-2 text-xs font-bold text-brand-900">
            {fill(t.candidatesTitle, { n: candidates.length })}
          </p>
            ...
            {candidates.map((c) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-slate-500">{[c.product, c.customer].filter(Boolean).join(" · ") || "-"} · {t.returnedShort} {c.returned_at || "-"}</span>
                </span>
              </Link>
            ))}
          </div>
      ...
      <form action={basePath} method="get" className="flex gap-2">
        ...
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <input name="q" defaultValue={q} placeholder={t.searchPlaceholder} className="h-9 w-full rounded-lg border border-slate-300 pl-9 pr-3 text-sm outline-none focus:border-brand-600" />
        </div>
        <button className="h-9 rounded-lg bg-brand-900 px-4 text-xs font-semibold text-white">{t.search}</button>
      </form>

      {rows.length === 0 ? (
        <p className="py-16 text-center text-sm text-slate-400">
          {total === 0 ? t.emptyNone : t.emptyFiltered}
        </p>
      ) : (
          ...
          <table className="w-full min-w-[820px] border-collapse text-[12px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-left text-[10px] uppercase tracking-wide text-slate-500">
                ...
                <th className="px-3 py-2 font-semibold">{t.colJob}</th>
              ...
              {rows.map((r) => (
                <tr key={r.claim_no} className={`border-b border-slate-100 hover:bg-slate-50 ${isClaimOpen(r.status) ? "" : "bg-slate-50/60"}`}>
                  ...
                  <td className="whitespace-nowrap px-3 py-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${tone(r.status)}`}>{claimStatusText(CL, r.claim_type, r.status)}</span>
                  </td>
